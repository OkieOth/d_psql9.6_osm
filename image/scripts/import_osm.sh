...
EOSQL
        ...
        for f in /osm/*; do
            case "$f" in
                *.osm.pbf) echo "$0: running $f"; osm2pgsql -c -S /usr/share/osm2pgsql/default.style --slim --unlogged -d $POSTGRES_DB -U $POSTGRES_USER --flat-nodes /opt/flatnodes/nodes.bin -C 1024 --number-processes 1 --cache-strategy sparse $f ; echo ;;
                *)        echo "$0: ignoring $f" ;;
            esac
            echo
        done
